...
const testimonials = [
  {
    ...
    role: "PR head, DBUU",
    quote:
      "The best community I've joined. The feedback culture is incredible—fast iteration, real care.",
    rating: 5,
    photo: "/assets/team-mates/ishita.jpg",
  },
  {
    id: "t2",
    name: "Shreya",
    role: "TFC Member, Frontend Developer",
    quote:
      "I shipped my first project because the learning was hands-on and premium. Truly transformative.",
    rating: 5,
    photo: "/assets/team-mates/shreya.jpg",
  },
  {
    id: "t3",
    name: "Om Giri",
    role: "Organizer, Head",
    quote:
      "A community that's calm, focused, and creative—exactly what I needed to grow.",
    rating: 5,
    photo: "/assets/team-mates/om.png",
  },
...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    const card = track.children[testimonialsIdx] as HTMLElement | undefined;
    if (!card) return;

    if (reducedMotion) {
      track.dataset.isAutoScrolling = "false";
      track.scrollLeft = card.offsetLeft;
      return;
    }

    // Prevent jerky transitions caused by overlapping smooth scroll calls.
    if (track.dataset.isAutoScrolling === "true") return;
    track.dataset.isAutoScrolling = "true";

    const targetLeft = card.offsetLeft;

    // Use rAF so layout is settled before animating.
    requestAnimationFrame(() => {
      // If we ever end up beyond max scroll (wrap-around edge), hard snap back.
      if (targetLeft > track.scrollWidth - track.clientWidth) {
        track.scrollLeft = Math.max(0, track.scrollWidth - track.clientWidth);
      }

      track.scrollTo({ left: targetLeft, behavior: "smooth" });

      // Re-enable after the smooth scroll duration.
      window.setTimeout(() => {
        if (!track) return;
        track.dataset.isAutoScrolling = "false";
      }, 1100);
    });
  }, [testimonialsIdx, reducedMotion]);

  return (
    <section className="c-section">
      <div className="c-sectionInner">
        <div
          className="c-testimonialsTrack"
          ref={trackRef}
        ...
        >
          {testimonials.map((t) => (
            <div key={t.id} className="c-testCard">
              <div
                className="c-testStars"
                aria-label={`${t.rating} out of 5 stars`}
              >
                {Array.from({ length: 5 }).map((_, i) => (
                  <span
                    key={i}
                    className={i < t.rating ? "is-on" : ""}
                  >
                    ★
                  </span>
                ))}
